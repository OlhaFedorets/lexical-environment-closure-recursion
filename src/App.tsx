...
function App() {
    ...
    //доступ к которой сохранен в переменной resultFunc. В переменной resultFunc сохраняется результат выполнения функции startEngine1!!!!
    //А этим результатом является функция func(вместе со ссылкой на свое внешнее лексическое окружение, которым является startEngineLE).
    // Именно поэтому мы можем теперь вызывать resultFunc(). Потому что ей присвоена функция!!
...
// const counter2 = counter();
    ...
    counter1() //2
    counter1() //3
    //здесь замыкание, так как лексическое значение (counterCreatorLE) не умирает, так как создается ссылка на функцию
    //путем ее присваивания переменной counter1. Переменной присваивается результат выполнения функции counterCreator,
    //а этим результатом является анонимная функция, которая выдает в консоль результат увеличения счетчика
